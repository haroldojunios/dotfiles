-- Converted from catppuccin.conf
--
-- CAVEAT: nothing in your other .conf files actually referenced these
-- $variables (look-and-feel.conf uses raw rgba() hex directly). If you have
-- an external script that greps catppuccin.conf for `$name = value` lines to
-- sync colors elsewhere (a common catppuccin-hyprland theming pattern), that
-- script will break once this becomes Lua-only, since there's no `$var`
-- syntax for it to find anymore. Point it at this file's `return` table
-- instead, or keep catppuccin.conf around purely as a color source.
--
-- Usage from another Lua file: local colors = require("colors")
--                               colors.blue -- "rgb(89b4fa)"
